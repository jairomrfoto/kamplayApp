...
import { Search, MapPin, Calendar, Star, ChevronRight, X } from 'lucide-react';
...
const ZONAS = [
  ...
  'Galicia', 'Castilla y León', 'Aragón', 'Islas Canarias', 'Islas Baleares', 'Internacional',
];
    ...
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map(i => (
        <Star
          key={i}
          size={13}
          className={i <= full ? 'text-amber-400 fill-amber-400' : i === full + 1 && half ? 'text-amber-300 fill-amber-300' : 'text-gray-200 fill-gray-200'}
        />
      ))}
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden hover:shadow-md transition-shadow duration-200 flex flex-col">
      ...
      <div className="p-5 flex flex-col flex-1 gap-3">
        ...
        {camp.description && camp.description.length > 120 && (
...
const CampDirectory = () => {
  ...
  useEffect(() => {
    getListedCamps().then(data => { setCamps(data); setLoading(false); }).catch(() => setLoading(false));
  }, []);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="bg-gradient-to-br from-orange-500 to-amber-600 text-white py-14 px-4">
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-orange-200 text-sm font-bold uppercase tracking-widest mb-4">Directorio</p>
          <h1 className="text-3xl sm:text-4xl font-extrabold mb-4">Descubre campamentos y campus</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          <div className="flex gap-1 bg-white border border-gray-200 rounded-xl p-1 shadow-sm">
            {[{ id: 'all' as const, label: 'Todos' }, { id: 'upcoming' as const, label: 'Próximos' }, { id: 'active' as const, label: 'En curso' }].map(({ id, label }) => (
